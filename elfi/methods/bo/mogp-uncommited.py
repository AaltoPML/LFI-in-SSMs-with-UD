...
class LMC(ApproximateGP):
    ...
    def __init__(self, inducing_points, num_latents, num_tasks, bounds, standardize=False, normalize=True, learn_inducing_locations=True):
        """Contructor
        Parameters
        ----------
        ...
        normalize : bool
            set to True to normalize input
        learn_inducing_locations : bool
            set to False to keep inducing location constant
        """
...
def train_lmc(lmc, train_x, train_y, num_epochs_max=1000, training_batch_size=None, verbose=False, **tkwargs):
    ...
    if lmc.normalize:
        train_x = normalize(train_x, bounds=lmc.bounds)
